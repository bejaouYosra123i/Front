...
const InboxPage = () => {
  ...
  return (
    <div className='container mx-auto p-6 bg-[#FFFFFF] min-h-screen'>
      <h1 className='text-3xl font-bold text-[#000000] mb-6'>Inbox</h1>
            ...
            <div key={item.id} className='grid grid-cols-8 gap-4 p-3 bg-[#FFFFFF] rounded-lg shadow-sm hover:bg-[#D3DCE6] transition-colors duration-200 border border-[#D3DCE6]'>
              ...
              <span>
                {item.senderUserName === user?.userName ? (
                  <MdOutput className='text-2xl text-[#ED1C24]' />
                ) : (
                  <MdInput className='text-2xl text-[#003087]' />
                )}
              </span>
              <span className='col-span-4 text-[#000000] break-words'>{item.text}</span>
